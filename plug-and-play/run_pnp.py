...
def main():
    ...
    with open(
        os.path.join(exp_path_root, exp_config.source_experiment_name, "args.json"), "r"
    ) as f:
        args = json.load(f)
        seed = args["seed"]
    ...
    negative_prompt = (
        source_prompt
        if exp_config.negative_prompt is None
        else exp_config.negative_prompt
    )

    seed_everything(seed)
    possible_ddim_steps = args["save_feature_timesteps"]
    assert (
        exp_config.num_ddim_sampling_steps in possible_ddim_steps
        or exp_config.num_ddim_sampling_steps is None
    ), f"possible sampling steps for this experiment are: {possible_ddim_steps}; for {exp_config.num_ddim_sampling_steps} steps, run 'run_features_extraction.py' with save_feature_timesteps = {exp_config.num_ddim_sampling_steps}"
    ddim_steps = (
        exp_config.num_ddim_sampling_steps
        if exp_config.num_ddim_sampling_steps is not None
        else possible_ddim_steps[-1]
    )
    ...
    outpaths = [
        os.path.join(
            f"{exp_path_root}/{exp_config.source_experiment_name}/translations",
            f"{exp_config.scale}_{translation_folder}",
        )
        for translation_folder in translation_folders
    ]
    out_label = (
        f"INJECTION_T_{exp_config.feature_injection_threshold}_STEPS_{ddim_steps}"
    )
    ...
    predicted_samples_paths = [
        os.path.join(outpath, f"predicted_samples_{out_label}") for outpath in outpaths
    ]
    ...
    def load_target_features():
        ...
        source_experiment_out_layers_path = os.path.join(
            exp_path_root, exp_config.source_experiment_name, "feature_maps"
        )
        source_experiment_qkv_path = os.path.join(
            exp_path_root, exp_config.source_experiment_name, "feature_maps"
        )
        ...
        iterator = tqdm(
            time_range, desc="loading source experiment features", total=total_steps
        )
        ...
        for i, t in enumerate(iterator):
            ...
            for output_block_idx, output_block_self_attn_map_injection_threshold in zip(
                self_attn_output_block_indices,
                output_block_self_attn_map_injection_thresholds,
            ):
                if i <= int(output_block_self_attn_map_injection_threshold):
                    output_q = torch.load(
                        os.path.join(
                            source_experiment_qkv_path,
                    ...
                    current_features[f"output_block_{output_block_idx}_self_attn_q"] = (
                        output_q
                    )
                    current_features[f"output_block_{output_block_idx}_self_attn_k"] = (
                        output_k
                    )

            for output_block_idx, feature_injection_threshold in zip(
                out_layers_output_block_indices, feature_injection_thresholds
            ):
                if i <= int(feature_injection_threshold):
                    output = torch.load(
                        os.path.join(
                            source_experiment_out_layers_path,
                            f"output_block_{output_block_idx}_out_layers_features_time_{t}.pt",
                        )
                    )
                    current_features[f"output_block_{output_block_idx}_out_layers"] = (
                        output
                    )
    ...
    start_code = (
        torch.load(start_code_path).cuda() if os.path.exists(start_code_path) else None
    )
            ...
            with model.ema_scope():
                ...
                if exp_config.scale != 1.0:
                    uc = model.get_learned_conditioning(
                        batch_size * [unconditional_prompt]
                    )
                ...
                for k, x_sample in enumerate(x_image_torch):
                    ...
                    img.save(
                        os.path.join(
                            outpaths[k], f"{out_label}_sample_{sample_idx}.png"
                        )
                    )
                    sample_idx += 1
