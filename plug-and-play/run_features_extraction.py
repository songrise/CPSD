...
def main():
    ...
    save_feature_timesteps = (
        exp_config.config.ddim_steps
        if exp_config.config.init_img == ""
        else exp_config.config.save_feature_timesteps
    )
    ...
    if os.path.exists(outpath):
        logging.warning(
            "Experiment directory already exists, previously saved content will be overriden"
        )
        if exp_config.config.init_img != "":
            with open(os.path.join(outpath, "args.json"), "r") as f:
                args = json.load(f)
            callback_timesteps_to_save = (
                args["save_feature_timesteps"] + callback_timesteps_to_save
            )
