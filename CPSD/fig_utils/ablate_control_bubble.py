...
import uuid
import omegaconf
import CPSD.utils.exp_utils as exp_utils
import CPSD.models.attn_injection as attn_injection
        ...
        for s_i, s_layer in enumerate(style_injection_layers):
            ...
            for i, cp in enumerate(content_prompt):
                ...
                for j, sp in enumerate(style_prompt):
                    ...
                    out_folder = f"/root/autodl-tmp/CPSD/vis_out/control/content_{c_i}_style_{s_i}/"
                    os.makedirs(out_folder, exist_ok=True)
                    content.save(
                        f"{out_folder}/{i}_{j}_content.png",
                    )
                    style.save(
                        f"{out_folder}/{i}_{j}_style.png",
                    )
                    final.save(
